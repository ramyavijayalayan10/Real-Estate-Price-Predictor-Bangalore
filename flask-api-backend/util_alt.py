...
def load_saved_artifacts():
    ...
    artifact_path = os.path.join(os.path.dirname(__file__), 'artifacts', 'columns_blr.json')
    with open(artifact_path, "r") as f:
        data = json.load(f)
        __data_columns = [col.lower() for col in data["data columns"]]
        __locations = __data_columns[3:]

    model_path = os.path.join(os.path.dirname(__file__), 'artifacts', 'Blr_house_price_prediction_model.pkl')
